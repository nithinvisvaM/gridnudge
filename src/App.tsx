...
// Real-world Indian grid peak pattern: 6-9 AM, 6-10 PM
// Uses sine-wave base + appliance duty cycles + thermal inertia model

class SmartMeterSimulator {
  constructor() {
    this.simMinute = 18 * 60; // Start at 6 PM (peak)
    this.acOn = true;
    this.washerOn = false;
    this.washerCycle = 0;
    this.acTemp = 26; // target temp
    this.ambientTemp = 34; // Chennai ambient
    this.fridgeCompressor = false;
    this.fridgeCycle = 0;
    this.lightsOn = true;
  }

  // Thermal inertia model for AC load
  getACLoad() {
    const hour = (this.simMinute / 60) % 24;
    const isPeak = (hour >= 6 && hour <= 9) || (hour >= 18 && hour <= 22);
    if (!this.acOn) return 0.05; // standby
    // AC works harder when ambient temp is high (peak hours = hotter)
  ...
  getWasherLoad() {
    ...
    this.washerCycle++;
    // Washing machine duty cycle: motor surge → steady → heat → spin
    if (this.washerCycle < 10) return 2.1 + Math.random() * 0.3; // startup surge
    if (this.washerCycle < 40) return 0.5 + Math.random() * 0.1; // agitate
    if (this.washerCycle < 60) return 1.8 + Math.random() * 0.2; // heat water
    if (this.washerCycle < 80) return 0.3 + Math.random() * 0.1; // rinse
    if (this.washerCycle < 90) return 1.6 + Math.random() * 0.2; // spin
    this.washerOn = false; this.washerCycle = 0; return 0; // done
  }

  getFridgeLoad() {
    // Fridge compressor duty cycle: 15 min on, 10 min off
    this.fridgeCycle++;
...
// Grid load simulation — based on India NLDC demand profile
function getGridLoad(simMinute) {
  ...
  const withNudge = raw - (morning * 0.32 + evening * 0.28); // ~30% shaved by nudges
  return { demand: +raw.toFixed(1), withNudge: +withNudge.toFixed(1), capacity: 240 };
}

// ─── Real Claude AI Nudge Generation ─────────────────────────────────────────
async function generateAINudge(usageSnapshot) {
...
Generate ONE specific, personalised AI nudge. Respond ONLY in this exact JSON format (no markdown, no extra text):
{
  ...
  "icon": "single emoji",
  "message": "specific actionable nudge under 25 words mentioning kW, savings in ₹, and points",
  ...
  try {
    ...
    return {
      ...
      message: `AC drawing ${usageSnapshot.ac}kW during peak. Raise thermostat 2°C → save ₹18 + earn 45 pts`,
...
function GaugeBar({ value, max, color, label, sub }) {
...
// ─── AI Nudge Card ────────────────────────────────────────────────────────────
function NudgeCard({ nudge, onAccept, onDismiss }) {
...
function HouseholdView() {
  ...
  const [live, setLive] = useState(() => simRef.current.tick());
  ...
  // Tick every 1.5 seconds — realistic 1-minute smart meter granularity sped up
  useEffect(() => {
    const iv = setInterval(() => {
      ...
      // Trigger AI nudge when in peak AND high load AND debounce
      if (reading.isPeak && reading.total > 2.8 && !nudgeDebounce.current) {
        nudgeDebounce.current = true;
        setTimeout(() => { nudgeDebounce.current = false; }, 20000); // nudge max every 20s
        setAiLoading(true);
  ...
  const LEADERBOARD = [
    { rank: 1, name: "Priya S.", zone: "Adyar", pts: 4820, saved: 312 },
    { rank: 2, name: "Rajan M.", zone: "T. Nagar", pts: 4650, saved: 289 },
    { rank: 3, name: "You", zone: "Velachery", pts: points, saved: Math.floor(points * 0.063), isUser: true },
    { rank: 4, name: "Kavitha R.", zone: "Anna Nagar", pts: 3720, saved: 228 },
    { rank: 5, name: "Suresh P.", zone: "Porur", pts: 3540, saved: 214 },
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 16 }}>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={history.slice(-30)} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                <defs>
                  {[["ac",C.blue],["washer",C.purple],["fridge",C.accent],["lights",C.warn]].map(([k,col]) => (
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div>
              ...
              <div style={{ color: C.muted, fontSize: 12, marginTop: 2 }}>
                Claude AI generates nudges from live smart meter data
              </div>
            </div>
          ...
          {nudges.length === 0 && !aiLoading && (
            <div style={{ textAlign: "center", padding: "40px 20px", color: C.muted }}>
              ...
              <div style={{ fontSize: 12 }}>AI nudges trigger automatically during peak hours when load exceeds 2.8 kW,<br/>or click "Get AI Nudge" above</div>
            </div>
          )}
          ...
          {nudges.length > 0 && (
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginTop: 8 }}>
              ...
              <div style={{ color: C.muted, fontSize: 10, marginTop: 5 }}>
                Industry avg: 18–22% · Your target: 35–45%
              </div>
            </div>
          )}
        </div>
...
function GridOperatorView() {
  ...
  const [liveGrid, setLiveGrid] = useState(() => getGridLoad(18 * 60));
  ...
  const sc = { critical: C.danger, high: C.warn, warn: C.warn, ok: C.blue, good: C.green };
    ...
    <div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 16 }}>
        ...
        <div style={{ color: C.muted, fontSize: 11, marginBottom: 14 }}>
          Physics-based India NLDC load profile · With vs Without GridNudge
        </div>
        ...
        <div style={{ display: "flex", gap: 20, marginTop: 10 }}>
          {[[C.danger,"Without Nudge"],[C.accent,"With GridNudge"]].map(([col, label]) => (
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 200, background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18 }}>
          ...
          {[["Peaker Plant Saved","₹48.2L",C.green],["Infrastructure Deferred","₹2.3 Cr",C.accent],["Carbon Credits","₹6.8L",C.blue]].map(([l,v,c])=>(
